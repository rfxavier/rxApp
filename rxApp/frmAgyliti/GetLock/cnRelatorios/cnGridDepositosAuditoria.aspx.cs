...
{
    public partial class cnGridDepositosAuditoria : System.Web.UI.Page
    {
        private ApplicationDbContext db;
        private ApplicationUserManager userManager;
        object ds;

        public cnGridDepositosAuditoria()
        {
        ...
        protected void ASPxGridView1_CustomUnboundColumnData(object sender, DevExpress.Web.ASPxGridViewColumnDataEventArgs e)
        {
            ...
            if (e.Column.FieldName == "DateTimeDiff")
            {
                TimeSpan span = (Convert.ToDateTime(e.GetListSourceFieldValue("trackCreationTime")) - Convert.ToDateTime(e.GetListSourceFieldValue("data_tmst_end_datetime")));

                //e.Value = String.Format("{0}d {1}h {2}m {3}s", span.Days.ToString().PadLeft(2, '0'), span.Hours.ToString().PadLeft(2, '0'), span.Minutes.ToString().PadLeft(2, '0'), span.Seconds.ToString().PadLeft(2, '0'));
                e.Value = String.Format("{1}h {2}m {3}s", span.Days.ToString().PadLeft(2, '0'), span.Hours.ToString().PadLeft(2, '0'), span.Minutes.ToString().PadLeft(2, '0'), span.Seconds.ToString().PadLeft(2, '0'));
            }
        }
