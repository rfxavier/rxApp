    ...
    public partial class cnGridDepositos : System.Web.UI.Page
    {
        ...
        protected void ASPxGridView1_DataBinding(object sender, EventArgs e)
        {
            ...
            if (User.IsInRole("User"))
            {
                ...
                ds = db.GetLockMessageViews.AsNoTracking().Where(m => (m.data_type == "1" || m.data_type == "2") &&  m.cod_loja == codLoja && m.trackCreationTime >= dateIni && m.trackCreationTime <= dateEnd).ToList();
            }
            else
            {
                ds = db.GetLockMessageViews.AsNoTracking().Where(m => (m.data_type == "1" || m.data_type == "2") && m.trackCreationTime >= dateIni && m.trackCreationTime <= dateEnd).ToList();
            }
